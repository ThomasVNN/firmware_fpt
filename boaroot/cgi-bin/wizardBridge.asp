...
<html>
	...
	<body topmargin="10" leftmargin="0">     <!--gleaf modified-->
		<form name="wzBridgeform" method="post" ACTION="/cgi-bin/wizardcomp.asp">
			...
			<INPUT TYPE="HIDDEN" NAME="disLanDHCP" VALUE="0"><!--cindy add to disable LAN dhcp when chose bridge mode 20190112-->
				...
				<%if tcWebApi_get("Wan_Common","TransMode","h") = "ATM" then%>
				  	...
				  	<tr>
				    		...
				    		<td class="tabdata" align="left">				    	
								...
								<option value="1483 Bridged Only LLC" <% if tcWebApi_get("Wan_PVC","ENCAP","h") = "1483 Bridged IP LLC" then asp_Write("selected") end if %>>
								<%tcWebApi_get("String_Entry","WANEncapsulation8Text","s")%>
								
								<option value="1483 Bridged Only VC-Mux" <% if tcWebApi_get("Wan_PVC","ENCAP","h") = "1483 Bridged IP VC-Mux" then asp_Write("selected") end if %>>
								<%tcWebApi_get("String_Entry","WANEncapsulation9Text","s")%>
					        	</select>				        
				      		</td>
				  	</tr>
				<%end if%>
