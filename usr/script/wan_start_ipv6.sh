	...
	if [ "$CONNECTION" != "Connect_Manually" ] ; then
		...
		if [ "$TCSUPPORT_CZ_GENERAL" != "" ] ;then
			...
			if [ "$TCSUPPORT_MULTISERVICE_ON_WAN" != "" ] && [ "$TCSUPPORT_WAN_PTM" != "" -o "$TCSUPPORT_WAN_ETHER" != "" ] && [ "$isPTMETHER" = "1" ]; then
        		...
        		PPP_PARAM="unit $i user $USERNAME password $PASSWORD nodetach holdoff 4 maxfail 0 usepeerdns lcp-echo-interval 15 lcp-echo-failure 4"
			fi
	fi 
if [ "$TCSUPPORT_WAN_GPON" != "" ] || [ "$TCSUPPORT_WAN_EPON" != "" ]; then
...
if [ "$TCSUPPORT_UNIQUEMAC" != "" ] ;then
	...
	if [ "$MTU" != "0" ] && [ "$MTU" != "" ] ; then
		...
		#20171013 modify mtu 1492 to 1450
		#20190919 change mtu to econet default 1492 to fix samsung j5 zalo packet fragment
		#20200108 change mtu to 1500 for jumbo frame
		PPP_PARAM="$PPP_PARAM mtu 1500 mru 1500"
	fi
...
if [ $ISP != "3" ] ; then
	if ! [ "$ISP" = "2" -a "$CONNECTION" = "Connect_Manually" ] ; then
		if [ "$BridgeInterface" = "Yes" ] ; then
#Foxconn alan add a flag to control PPPOE Relay (20181203)
			if [ "$PPPOERelay" = "Yes" ] ; then
