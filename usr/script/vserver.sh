...
if [ "$TCSUPPORT_MULTISERVICE_ON_WAN" != "" ] && [ "$TCSUPPORT_WAN_PTM" != "" -o "$TCSUPPORT_WAN_ETHER" != "" ]; then
	...
	if [ $# != 4 ] ; then
		echo "usage: $0 [add/del] [PVC#] [Rule#] [WanIP#]"
		exit 0
	fi
fi
...
if [ "$SERVER_IP" = "0.0.0.0" ] ; then
	...
	if [ "$PROTOCOL" = "TCP" ] ;then
		...
		iptables -t nat $OPT VS_PRE$PVC -p udp --dport $EXT_SPORT:$EXT_EPORT -i $WAN_IF -j DNAT --to $INT_IP:$INT_SPORT-$INT_EPORT
			...
			# foxconn howard add for LAN client access virtual server
			if [ "$WANIP" != "0.0.0.0" ] ;then
			iptables -t nat $OPT VS_PRE$PVC -p tcp -i br0 -d $WANIP --dport $EXT_SPORT:$EXT_EPORT -j DNAT --to $INT_IP:$INT_SPORT-$INT_EPORT
			iptables -t nat $OPT VS_PRE$PVC -p udp -i br0 -d $WANIP --dport $EXT_SPORT:$EXT_EPORT -j DNAT --to $INT_IP:$INT_SPORT-$INT_EPORT
			
			iptables $OPT VS_FOWARD$PVC -p tcp -i br0 -d $INT_IP -j ACCEPT
			iptables $OPT VS_FOWARD$PVC -p udp -i br0 -d $INT_IP -j ACCEPT
			
			iptables -t nat $OPT VS_POS$PVC -p tcp -d $INT_IP --dport $INT_SPORT:$INT_EPORT -j MASQUERADE
			iptables -t nat $OPT VS_POS$PVC -p udp -d $INT_IP --dport $INT_SPORT:$INT_EPORT -j MASQUERADE
			fi
		else
