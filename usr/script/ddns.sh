...
#Foxconn alan change from "no-ip" to "noip" to DDNS support noip
SERVER_NOIP=www.noip.com
#Foxconn alan add for support DDNS "changeip"(2017.8.29)
SERVER_CHANGEIP=www.changeip.com 
...
#Foxconn alan add for support DDNS "changeip"(2017.8.29)
	elif [  "$SERVERNAME" = "$SERVER_CHANGEIP" ] ; then
		echo "service-type=changeip" >>$IPUPDATE_CONF
	else
